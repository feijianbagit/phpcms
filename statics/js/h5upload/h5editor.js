function h5upload(sysfilename, uploadid, name, textareaid, funcName, args, module, catid, authkey, syseditor) {
	...
	diag.onOk = function(){
		...
		if (funcName=='thumb_images') {
			...
			$('#fileupload_'+textareaid).find('.mpreview').html((IsImg(in_content[0]) && $('#fileupload_'+textareaid).find('#crop_'+textareaid).attr('class') ? '<a href="javascript:crop_cut_'+textareaid+'('+in_id[0]+');"><i class="fa fa-cut"></i></a>' : ''));
			...
			$.each( contents, function(i, n) {
				...
				str += '<tr class="template-download files_row"><td style="text-align:center;width: 80px;"><div class="files_row_preview preview"><a href="javascript:preview(\''+n+'\');"><img src="'+(IsImg(n) ? n : get_web_dir()+'api.php?op=icon&fileext='+n.substring(n.lastIndexOf('.')+1))+'"></a></div></td><td class="files_show_info"><div class="row"><div class="col-md-12 files_show_title_html"><input class="form-control files_row_title" type="text" name="'+textareaid+'_title[]" value="'+filename+'"><input type="hidden" class="files_row_id" name="'+textareaid+'_id[]" value="'+(id ? id : n)+'"></div><div class="col-md-12 files_show_description_html"><textarea class="form-control files_row_description" name="'+textareaid+'_description[]"></textarea></div></div></td><td style="text-align:center;width: 80px;"><label><button onclick="dr_file_remove(this, \''+textareaid+'\')" type="button" class="btn red file_delete btn-sm"><i class="fa fa-trash"></i></button></label></td></tr>';
			});
			$('#'+textareaid).html(str);
			...
			$('#'+textareaid).val(in_id[0]);
			$('#dr_'+textareaid+'_files_row').html('<div class="files_row_preview preview"><a href="javascript:preview(\''+in_content[0]+'\');"><img src="'+(IsImg(in_content[0]) ? in_content[0] : get_web_dir()+'api.php?op=icon&fileext='+in_content[0].substring(in_content[0].lastIndexOf('.')+1))+'"></a></div>'+(IsImg(in_content[0]) && $('#fileupload_'+textareaid).find('#crop_'+textareaid).attr('class') ? '<div class="mpreview"><a href="javascript:crop_cut_'+textareaid+'('+in_id[0]+');"><i class="fa fa-cut"></i></a></div>' : ''));
			$('#fileupload_'+textareaid).find('.'+textareaid+'-delete').show();
