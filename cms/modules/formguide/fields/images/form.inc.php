	function images($field, $value, $fieldinfo) {
		...
		if($show_type && defined('IN_ADMIN')) {
			$string .= $str."<input type='button' class='button' onclick=\"javascript:flashupload('{$field}_images', '".L('attachment_upload')."','{$field}','change_thumbs','{$upload_number},{$upload_allowext},{$isselectimage},,,,{$image_reduce}','content','$this->catid','{$authkey}')\"/ value='".L('select_pic')."'>";
		} else {
			$string .= $str."<input type='button' class='button' onclick=\"javascript:flashupload('{$field}_images', '".L('attachment_upload')."','{$field}','change_images','{$upload_number},{$upload_allowext},{$isselectimage},,,,{$image_reduce}','content','$this->catid','{$authkey}')\"/ value='".L('select_pic')."'>";
		}
		return $string;
	}