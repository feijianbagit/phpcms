...
<?php foreach($filed_list[$r['id']] as $f){ ?>
	<tr id="tr<?php echo $j ?>">
		...
		<td align="center"><label><input name="postdata[<?php echo $j ?>][name]" type='text' value='<?php echo $f['name']?>' class="input-text" style="width:auto!important;" /></label></td>
		<td align="center"><label><textarea name="postdata[<?php echo $j ?>][val]" class="input-text" style="width:auto!important;"><?php echo $f['val']?></textarea><label></td>
...
<div class="portlet-body form myfooter">
	<div class="form-actions text-center">
		<button type="button" onclick="dr_ajax_submit('?m=customfield&c=customfield&a=manage_save', 'myform', '2000', '<?php echo dr_now_url();?>')" class="btn green"> <i class="fa fa-save"></i> <?php echo L('cm_save')?></button>
	</div>
</div>
</div>
...
function addTr(pid){
	...
	trHtml  +=	"</td>";
	trHtml  +=	"<td align='center'><label><input name='postdata[" + addnum + "][name]' type='text' value='' class='input-text' style='width:auto!important;' /></label></td>";
	trHtml  +=	"<td align='center'><label><textarea name='postdata[" + addnum + "][val]' class='input-text' style='width:auto!important;' rows='2'></textarea></label></td>";
