...
class category extends admin {
	...
	public function init () {
		...
		if(!empty($result)) {
			foreach($result as $r) {
				...
				$r['str_manage'] .= '<a href="javascript:addedit(\'?m=admin&c=category&a=add&parentid='.$r['catid'].'&menuid='.$_GET['menuid'].'&s='.$r['type'].'&pc_hash='.$_SESSION['pc_hash'].'\', \''.L('add_sub_category').'\')">'.L('add_sub_category').'</a> | ';
				
				$r['str_manage'] .= '<a href="javascript:addedit(\'?m=admin&c=category&a=edit&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'&type='.$r['type'].'&pc_hash='.$_SESSION['pc_hash'].'\', \''.L('edit').'\')">'.L('edit').'</a> | <a href="javascript:confirmurl(\'?m=admin&c=category&a=delete&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'\',\''.L('confirm',array('message'=>addslashes($r['catname']))).'\')">'.L('delete').'</a> | <a href="?m=admin&c=category&a=remove&catid='.$r['catid'].'&pc_hash='.$_SESSION['pc_hash'].'">'.L('remove','','content').'</a>';
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$end_str = $old_end =  '<script type="text/javascript">Dialog.warn("'.L("operation_success").L("edit_following_operation").'",function(){parent.right.location = "?m=admin&c=category&a=public_cache&menuid=43&module=admin";});</script>';
			if(!isset($_POST['batch_add']) || empty($_POST['batch_add'])) {
	...
	public function edit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			showmessage(L('operation_success').'<script type="text/javascript">Dialog.warn("'.L("operation_success").L("edit_following_operation").'",function(){parent.right.location = "?m=admin&c=category&a=public_cache&menuid=43&module=admin";});</script>');
		} else {
			$show_header = $show_dialog = '';
			//获取站点模板信息
			pc_base::load_app_func('global');
