...
class admin {
	...
	public function cleck_edit_member($uid) {
		...
		if (cleck_admin(param::get_session('roleid'))) {
			// 超管不验证
			return true;
		} elseif (param::get_session('userid') == $uid) {
			// 自己不验证
			return true;
	...
	final public function check_url() {
		if(ROUTE_M =='admin' && in_array(ROUTE_C, array('index', 'admin_manage')) && in_array(ROUTE_A, array(SYS_ADMIN_PATH, 'public_main', 'init', 'public_edit_pwd', 'fclient'))) {
			return true;
		} else {
			if (!HTTP_REFERER) {
