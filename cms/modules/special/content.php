...
class content extends admin {
	...
	public function add() {
		...
		if ($_POST['dosubmit'] || $_POST['dosubmit_continue']) {
			...
			if ($_POST['dosubmit']) showmessage(L('content_add_success'), HTTP_REFERER, '', '', 'setTimeout("this.close()", 2000)');
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			$html->_index($_GET['specialid'], 20, 5);
			$html->_list($info['typeid'], 20, 5);
			showmessage(L('content_edit_success'), HTTP_REFERER, '', '', 'setTimeout("this.close()", 2000)');
		} else {
	...
	public function init() {
		...
		$pages = $this->db->pages;
		$big_menu = array(array('javascript:openwinx(\'?m=special&c=content&a=add&specialid='.$_GET['specialid'].'\',\'\');void(0);', L('add_content')), array('javascript:artdialog(\'import\',\'?m=special&c=special&a=import&specialid='.$_GET['specialid'].'\',\''.L('import_content').'\',700,500);void(0);', L('import_content')));
