...
class member extends admin {
	...
	function add() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$_POST['info']['encrypt'] = create_randomstr(6);
			$info['password'] = password($_POST['info']['password'], $_POST['info']['encrypt']);
			$info['encrypt'] = $_POST['info']['encrypt'];
			$info['nickname'] = $_POST['info']['nickname'];
			$info['email'] = $_POST['info']['email'];
			$info['mobile'] = $_POST['info']['mobile'];
			$info['groupid'] = $_POST['info']['groupid'];
			$info['point'] = $_POST['info']['point'];
			$info['modelid'] = $_POST['info']['modelid'];
			$info['vip'] = $_POST['info']['vip'];
			$info['regdate'] = $_POST['info']['lastdate'] = SYS_TIME;
	...
	public function alogin_index() {
		...
		$r = $this->db->get_one(array('userid'=>$uid));
		if ($r) {
			//如果用户被锁定
			if($r['islock']) {
		...
		showmessage(L('正在授权登录此用户...'), 'index.php?m=member');exit;
	}
	...
	public function deleteavatar($uid) {
		...
		if ($memberinfo) {
			...
			if($attachment->delete(array('aid'=>$memberinfo['avatar']))) {
	...
	function memberinfo() {
		...
		foreach($model_fieldinfo as $k=>$v) {
			if($v['formtype'] == 'image') {
				$member_modelinfo[$k] = "<a href='.$member_modelinfo[$k].' target='_blank'><img src='.$member_modelinfo[$k].' height='40' widht='40' onerror=\"this.src='/statics/images/member/nophoto.gif'\"></a>";
			} elseif($v['formtype'] == 'images') {
				$tmp = string2array($member_modelinfo[$k]);
				$member_modelinfo[$k] = '';
				if(is_array($tmp)) {
					foreach ($tmp as $tv) {
						$member_modelinfo[$k] .= " <a href='$tv[url]' target='_blank'><img src='$tv[url]' height='40' widht='40' onerror=\"this.src='/statics/images/member/nophoto.gif'\"></a>";
					}
					unset($tmp);
				}
