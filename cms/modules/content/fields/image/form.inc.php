	...
	var diag = new Dialog({id:'crop',title:'".L('cut_the_picture','','content')."',url:'index.php?m=content&c=content&a=public_crop&module=content&catid='+catid+'&spec=2&picurl='+window.btoa(unescape(encodeURIComponent(id)))+'&input=$field&preview=".($show_type && defined('IN_ADMIN') ? $field."_preview" : '')."',width:770,height:510,modal:true});diag.onOk = function(){\$DW.dosbumit();return false;};diag.onCancel=function() {\$DW.close();};diag.show();
};</script>";
			}
		$authkey = upload_key("1,$upload_allowext,$isselectimage,$images_width,$images_height,$watermark,$image_reduce");
