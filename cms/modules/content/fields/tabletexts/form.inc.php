	function tabletexts($field, $value, $fieldinfo) {
			...
			if(is_array($value)) {
				foreach($value as $_k=>$_v) {
					...
					for ($x=1; $x<=count($columns); $x++) {
						$list_str .="<td><input type='text' name='".$field."_".$x."[]' value='".$_v[$field."_".$x]."' class='input-text' style='width:100%; padding:0;'></td>";
					}
		...
		function add'.$field.'(id){
			...
			for($cols=1; $cols<=count($columns); $cols++){
				$string .='<td><input type=\'text\' name=\''.$field.'_'.$cols.'[]\' value=\'\' class=\'input-text\' style=\'width:100%; padding:0;\'></td>';
			}
		...
		$string .="<th align='left' style='text-align:left; padding:0 0 0 12px; border-bottom: 1px solid #d5dfe8; width:170px;'>操作</th></tr></thead><tbody>";
		$string .= $list_str;
