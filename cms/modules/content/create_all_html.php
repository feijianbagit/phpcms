		...
		if(isset($_POST['dosubmit'])) {
			...
			if($modelid) {
				...
				if($pages > $page) {
					...
					$forward = $start ? "?m=content&c=create_all_html&a=update_urls&type=$type&dosubmit=1&first=$first&fromid=$fromid&toid=$toid&fromdate=$fromdate&todate=$todate&pagesize=$pagesize&page=$page&pages=$pages&total=$total&modelid=$modelid" : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/", "&page=$page&pages=$pages&total=$total", $http_url);
				} else {
					delcache('url_show_'.$_SESSION['userid'],'content');
					$message = L('create_update_success');
					$forward = '?m=content&c=create_all_html&a=update_urls';
				}
				...
				if(!isset($set_catid)) {
					...
					$forward = "?m=content&c=create_all_html&a=update_urls&set_catid=1&pagesize=$pagesize&dosubmit=1";
					showmessage($message,$forward,200);
				}
				$catid_arr = getcache('update_url_catid'.'-'.$this->siteid.'-'.$_SESSION['userid'],'content');
				$autoid = $autoid ? intval($autoid) : 0;
				if(!isset($catid_arr[$autoid])) showmessage(L('create_update_success'),'?m=content&c=create_all_html&a=update_urls',200);
	...
	public function category() {
		if(isset($_POST['dosubmit'])) {
			...
			do {
				...
				$page++;
				$j++;
				$total_number = isset($total_number) ? $total_number : defined('PAGES');
