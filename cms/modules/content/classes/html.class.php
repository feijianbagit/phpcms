...
class html {
		...
		if($paginationtype!=0) {
			...
			if($CONTENT_POS !== false) {
				...
				foreach ($pageurls as $page=>$urls) {
					...
					if(dr_site_info('mobilemode', $this->siteid)==1 && dr_site_info('mobilehtml', $this->siteid)==1) {
	...
	public function category($catid, $page = 0, $maxsize = 0) {
		...
		//生成地址
		if($setting['create_to_html_root']) $parentdir = '';
		$isdomain = '0';
		...
		if($type==0) {
			...
			if($pagenumber > $pagesize) {
				$pages = category_pages($pagenumber,$page,$pagesize,$categoryurl);
			}
			pc_base::load_sys_class('service')->assign(array(
				'SEO' => $SEO,
				'top_parentid' => $top_parentid,
				'fix_html_now_url' => $page == 1 ? $categoryurl[0] : str_replace('{page}', $page, $categoryurl[1]),
			));
		} else {
			//单网页
		...
		if ($type != 2 && $child && $setting['getchild']) {
			...
			// 传入模板
			pc_base::load_sys_class('service')->assign($CAT);
			...
			if(dr_site_info('mobilehtml', $this->siteid)==1) {
				...
				pc_base::load_sys_class('service')->assign(array(
					'pages' => $pages,
					'fix_html_now_url' => $page == 1 ? $url_arr[0] : str_replace('{page}', $page, $url_arr[1]),
				));
				ob_start();
				pc_base::load_sys_class('service')->display('mobile',$template);
				$this->createhtml($mobilefile);
