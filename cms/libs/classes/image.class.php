...
class image {
    ...
    {
        return ($this->dr_count($this->error_msg) > 0) ? $open.implode($close.$open, $this->error_msg).$close : '';
    }
    ...
    public function thumb($img, $width = 0, $height = 0, $water = 0, $mode = 'auto', $webimg = 0) {
        ...
        if (is_numeric($img)) {
            ...
            $attach_db = $att_db->get_one(array('aid'=>$img));
            if (!$attach_db) {
                //log_message('error', '图片id['.$img.']不存在，thumb函数无法调用');
                return IMG_PATH.'nopic.gif';
            } elseif (!in_array($attach_db['fileext'], array('gif', 'png', 'jpeg', 'jpg'))) {
                //log_message('error', '图片id['.$img.']扩展名不符合条件，thumb函数无法调用');
                return IMG_PATH.'nopic.gif';
            } else {
                $attach = array(
                    'id' => md5($upload_url.$attach_db['filepath']),
                    'url' => $upload_url.$attach_db['filepath'],
                    'file' => $upload_path.$attach_db['filepath'],
                    'imgurl' => $attach_db['filepath'],
                    'remote' => 'webimg',
                    'fileext' => $attach_db['fileext'],
                );
            }
        } else {
            $attach = array(
                'id' => md5($img),
                'url' => $img,
                'file' => $upload_path.str_replace($upload_url, '', $img),
                'imgurl' => str_replace($upload_url, '', $img),
                'remote' => 'webimg',
        ...
        //list($cache_path, $cache_url) = array($upload_path, $upload_url);

        // 图片缩略图文件
        if(!is_file($attach['file']) && $webimg) {
            list($cache_path, $cache_url) = array($upload_path.'thumb/', $upload_url.'thumb/');
            $cache_file = $attach['id'].'/'.$width.'x'.$height.($water ? '_water' : '').($mode ? '_'.$mode : '').'.jpg';
        } else {
            $cache_file = dirname($attach['imgurl']).'/thumb_'.$width.'_'.$height.'_'.basename($attach['imgurl']);
            list($cache_path, $cache_url) = array($upload_path, $upload_url);
        }
        ...
        // 创建缓存目录
        $this->dr_mkdirs($cache_path.dirname($cache_file));

        // 本地存储的原始图片
        $file = $attach['file'];
        if ($attach['remote'] && $attach['url']) {
            // 远程图片
            if(!is_file($file) && $webimg) {
                $data = $this->dr_catcher_data($attach['url'], 10);
                if (!$data) {
                    //log_message('error', '图片['.$attach['url'].']无法获取远程附件数据，thumb函数无法调用');
                    return $attach['url'];
                }
                $this->dr_mkdirs(CACHE_PATH.'caches_attach/');
                $file = CACHE_PATH.'caches_attach/'.$attach['id'].'.'.$attach['fileext'];
                if (!file_put_contents($file, $data)) {
                    //log_message('error', '图片['.$attach['url'].']无法写入附件缓存目录，thumb函数无法调用');
                    return $attach['url'];
                }
            }
        } elseif (!is_file($file)) {
        ...
        if ($limit && $limit - $max < 0) {
            //log_message('error', '图片['.$attach['url'].']分辨率太大导致服务器内存溢出，无法进行缩略图处理，已按原图显示');
            return $attach['url']; // 原样输出
        }
    ...
    // 兼容统计
    public function dr_count($array_or_countable, $mode = COUNT_NORMAL){
     ...
     * @return    bool|void
     */
    public function dr_mkdirs($dir, $null = true) {
     ...
     * @return    string
     */
    public function dr_catcher_data($url, $timeout = 0) {
