...
class model {
	...
	final public function insert($data, $return_insert_id = false, $replace = false) {
		...
		$return_insert_id && $id = $this->insert_id();
		$return_insert_id && !$id && $id = intval($data[$this->get_primary($this->table_name)]);

		if ($return_insert_id && !$id) {
			log_message('debug', $this->table_name.': 主键获取失败');
			return $this->_return_error($this->table_name.': 主键获取失败');
		}
		return $id;
	}
