...
class service {
    ...
    public function display($module = 'content', $template = 'index', $style = '') {
        ...
        if ((IS_DEV || (IS_ADMIN && SYS_DEBUG))
            ...
            && !IS_AJAX) {
            $is_dev = 1;
            echo '<!--当前页面的模板文件是：'.str_replace(array(CACHE_PATH.'caches_template'.DIRECTORY_SEPARATOR, '.php'), array(TPLPATH, '.html'), $_view_file).' （本代码只在开发者模式下显示）-->'.PHP_EOL;
        }
    ...
    public function load_view_file($file) {
        ...
        $this->_view_files[$file] = [
            ...
            'path' => str_replace(array(CACHE_PATH.'caches_template'.DIRECTORY_SEPARATOR, '.php'), array(TPLPATH, '.html'), $file),
        ];
