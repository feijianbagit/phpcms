...
class attachment {
	 ...
	 */
	function upload($field, $alowexts = '', $maxsize = 0, $overwrite = 0,$thumb_setting = array(), $watermark_enable = 1, $image_reduce = '') {
		...
		foreach($uploadfiles as $k=>$file) {
			...
			$temp_filename = substr(md5(SYS_TIME.$file['name'].uniqid()), rand(0, 20), 15).'.'.$fileext;
			...
			if(@$upload_func($file['tmp_name'], $savefile)) {
				...
				$image = new image();
				$config = $this->_get_site_setting($this->siteid);
				if($thumb_enable) {
					$image->thumb($savefile,$thumb_setting[0],$thumb_setting[1]);
				}
				// 图片处理
				if (is_image($savefile)) {
					// 图片压缩处理
					if ($image_reduce) {
						$image->reduce($savefile, $image_reduce);
					}
					// 强制水印
					if($watermark_enable && $config['watermark_enable']) {
						$config['source_image'] = $savefile;
	...
	function download($field, $value,$watermark = '0',$ext = 'gif|jpg|jpeg|bmp|png', $absurl = '', $basehref = '')
	{
		...
		foreach($remotefileurls as $k=>$file) {
			...
			$filename = substr(md5(SYS_TIME.uniqid().$value), rand(0, 20), 15).'.'.$filename;
	...
	*/
	...
	function uploaderror($id = 0)	{
		file_put_contents(CMS_PATH.'xxx.txt', $id);
	}
