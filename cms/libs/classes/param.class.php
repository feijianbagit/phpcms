	...
	public static function redirect($url, $auto = false) {
		...
		if ($url != FC_NOW_URL) {
			if (!$auto) {
				...
				// 自动识别
				if (IS_ADMIN) {
					dr_admin_msg(1, '开发者模式：<br>当前URL['.dr_now_url().']<br>自动识别为['.$url.']<br>若不需要识别功能可在后台-设置-站点管理-手机设置-关闭自动识别<br>正在自动跳转本身地址（关闭开发者模式时即可自动跳转）', $url, 9);
				} else {
					dr_msg(1, '开发者模式：<br>当前URL['.dr_now_url().']<br>自动识别为['.$url.']<br>若不需要识别功能可在后台-设置-站点管理-手机设置-关闭自动识别<br>正在自动跳转本身地址（关闭开发者模式时即可自动跳转）', $url, 9);
				}
			}
