...
// 获取内容的搜索词
function dr_get_content_kws($value, $siteid = '', $mid = '') {
	if (is_array($value)) {
		return $value;
	} elseif (!$value) {
		return [];
	...
	foreach ($tag as $t) {
		$t = trim($t);
		if ($t) {
			$rt[$t] = $mid ? tag_url($t,$siteid) : APP_PATH.'index.php?m=search&c=index&a=init&typeid=0&siteid='.$siteid.'&q='.$t;
		}
	}
...
function dr_get_files($value, $limit = '') {
	...
	foreach ($value as $i => $file) {
		if ($file) {
			$data[] = [
				'url' => dr_get_file($file['id']), // 对应文件的url
				'file' => $file['id'], // 对应文件或附件id
...
function pages($num, $curr_page, $perpage = 10, $urlrule = '', $array = array(),$setpages = 10) {
	...
	$multipage = '';
	if($num > $perpage) {
		$multipage = pc_base::load_sys_class('input')->page(pageurl($urlrule, $curr_page, $array), $num, $perpage, $curr_page, $first_url);
	}
	return $multipage;
}
...
function get_sql_catid($file = 'module/category-1-data', $catid = 0, $module = 'commons') {
