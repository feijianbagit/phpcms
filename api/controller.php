...
$CONFIG = array (
    ...
    'userid'=>$userid,
    'is_wm'=>intval($_GET['is_wm']),
    'is_esi'=>intval($_GET['is_esi']),
    'image_reduce'=>intval($_GET['image_reduce']),
    /* 上传图片配置项 */
    ...
    'imageUrlPrefix'=>substr(APP_PATH,0,-strlen(WEB_PATH)), /* 图片访问路径前缀 */
    'imagePathFormat'=>str_replace(CMS_PATH,WEB_PATH,pc_base::load_config('system','upload_path')).$site_setting['filename'], /* 上传保存路径,可以自定义保存路径和文件名格式 */
    ...
    'scrawlPathFormat'=>str_replace(CMS_PATH,WEB_PATH,pc_base::load_config('system','upload_path')).$site_setting['filename'], /* 上传保存路径,可以自定义保存路径和文件名格式 */
    'scrawlMaxSize'=>$site_setting['upload_maxsize'] * 1000, /* 上传大小限制，单位B */
    'scrawlUrlPrefix'=>substr(APP_PATH,0,-strlen(WEB_PATH)), /* 图片访问路径前缀 */
    ...
    'snapscreenUrlPrefix'=>substr(APP_PATH,0,-strlen(WEB_PATH)), /* 图片访问路径前缀 */
    'snapscreenPathFormat'=>str_replace(CMS_PATH,WEB_PATH,pc_base::load_config('system','upload_path')).$site_setting['filename'], /* 上传保存路径,可以自定义保存路径和文件名格式 */

    /* 抓取远程图片配置 */
    'catcherLocalDomain'=>array('127.0.0.1', 'localhost', 'img.baidu.com'),
    'catcherActionName'=>'catchimage', /* 执行抓取远程图片的action名称 */
    'catcherFieldName'=>'source', /* 提交的图片列表表单名称 */
    'catcherPathFormat'=>str_replace(CMS_PATH,WEB_PATH,pc_base::load_config('system','upload_path')).$site_setting['filename'], /* 上传保存路径,可以自定义保存路径和文件名格式 */
    'catcherUrlPrefix'=>substr(APP_PATH,0,-strlen(WEB_PATH)), /* 图片访问路径前缀 */
    ...
    'videoPathFormat'=>str_replace(CMS_PATH,WEB_PATH,pc_base::load_config('system','upload_path')).$site_setting['filename'], /* 上传保存路径,可以自定义保存路径和文件名格式 */
    'videoUrlPrefix'=>substr(APP_PATH,0,-strlen(WEB_PATH)), /* 视频访问路径前缀 */
    ...
    "filePathFormat"=>str_replace(CMS_PATH,WEB_PATH,pc_base::load_config('system','upload_path')).$site_setting['filename'], /* 上传保存路径,可以自定义保存路径和文件名格式 */
    "fileUrlPrefix"=>substr(APP_PATH,0,-strlen(WEB_PATH)), /* 文件访问路径前缀 */
    "fileMaxSize"=>$site_setting['fileMaxSize'] * 1000, /* 上传大小限制，单位B，默认50MB */
    "fileAllowFiles"=>$fileAllowFiles, /* 上传文件格式显示 */

    /* 列出指定目录下的图片 */
    'imageManagerActionName'=>'listimage', /* 执行图片管理的action名称 */
    'imageManagerListPath'=>str_replace(CMS_PATH,WEB_PATH,pc_base::load_config('system','upload_path')).'', /* 指定要列出图片的目录 */
    'imageManagerListSize'=>$site_setting['imageManagerListSize'], /* 每次列出文件数量 */
    'imageManagerUrlPrefix'=>substr(APP_PATH,0,-strlen(WEB_PATH)), /* 图片访问路径前缀 */
    'imageManagerInsertAlign'=>'none', /* 插入的图片浮动方式 */
    'imageManagerAllowFiles'=>$imageAllowFiles, /* 列出的文件类型 */

    /* 列出指定目录下的文件 */
    'fileManagerActionName'=>'listfile', /* 执行文件管理的action名称 */
    'fileManagerListPath'=>str_replace(CMS_PATH,WEB_PATH,pc_base::load_config('system','upload_path')).'', /* 指定要列出文件的目录 */
    'fileManagerUrlPrefix'=>substr(APP_PATH,0,-strlen(WEB_PATH)), /* 文件访问路径前缀 */
    ...
    'videoManagerListPath'=>str_replace(CMS_PATH,WEB_PATH,pc_base::load_config('system','upload_path')).'', /* 指定要列出文件的目录 */
    'videoManagerUrlPrefix'=>substr(APP_PATH,0,-strlen(WEB_PATH)), /* 文件访问路径前缀 */
